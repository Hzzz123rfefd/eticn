...
if __name__ == "__main__":
    ...
    M = 128
    x = torch.rand(1, M, 32, 32)*10
    scales = torch.rand(1, 2 * M, 32, 32)
    means = torch.rand(1, 2 * M, 32, 32)
    weights = torch.rand(1, 2 * M, 32, 32)
    (rv, abs_max, zero_bitmap), y_quantized = gaussian_mixture_conditional.compress(x,scales,means,weights)
    print(rv)

    x_hat = gaussian_mixture_conditional.decompress(rv,abs_max,zero_bitmap,scales,means,weights)

    print("x:",x)
    print("x_hat:",x_hat)

    # M = 128
    # gaussian = GaussianConditional(None)

    # scale_table = get_scale_table()
    # gaussian.update_scale_table(scale_table, force=False)

    # x = torch.rand(1, M, 32, 32)
    # scales = torch.rand(1, M, 32, 32)
    # means = torch.rand(1, M, 32, 32)
    # indexes = gaussian.build_indexes(scales)
    # strings = gaussian.compress(x,indexes)
    # print(strings)